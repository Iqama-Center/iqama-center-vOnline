            ...
            case 'admin':
                query = `
                    SELECT 
                        ...
                        (SELECT CASE 
                            WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payments') 
                            THEN (SELECT COALESCE(SUM(amount), 0) FROM payments WHERE status = 'paid')
                            ELSE 0 
                         END) as total_revenue,
                        
                        (SELECT CASE 
                            WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payments') 
                            THEN (SELECT COALESCE(SUM(amount), 0) FROM payments WHERE status IN ('due', 'pending_review', 'late'))
                            ELSE 0 
    ...
    try {
        ...
        if (role) {
            whereConditions.push(`(
                c.participant_config::text LIKE $${++paramCount}
                OR c.details->>'target_roles' LIKE $${++paramCount}
            )`);
        ...
        // Price filtering
        if (price_min) {
            whereConditions.push(`(c.details->>'cost')::numeric >= $${++paramCount}`);
