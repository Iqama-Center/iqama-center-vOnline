...
export async function generateEnhancedTasksForCourse(courseId, courseData, enrollments, taskConfig = {}) {
        ...
        for (const role of config.roles) {
            ...
            for (const template of templates.daily) {
                await client.query(`
                    ...
                    [
                        courseId, levelNumber, template.type, template.type, template.title,
                        template.description, 24, template.maxScore,
                        template.instructions, role, 'daily', 24, template.gradeReduction || 10
                    ]
                );
            }
            ...
            for (const template of templates.fixed) {
                await client.query(`
                    ...
                    [
                        ...
                        template.instructions, role, 'fixed', template.dueAfterDays || 3, template.priority || 'medium'
                    ]
                );
            }
        }
